...
router.get(
  ...
  authenticateToken,
  requireRole(['artisan']),
  ArtisanController.getPerformanceMetrics,
);
...
router.post('/schedule', authenticateToken, requireRole(['artisan']), [
  ...
  body('isAvailable').isBoolean()
], ArtisanController.setSchedule);

// Get upcoming jobs
router.get('/upcoming-jobs', authenticateToken, requireRole(['artisan']), [
  query('limit').optional().isInt({ min: 1, max: 50 })
], ArtisanController.getUpcomingJobs);
 

// Get artisan tools & equipment
router.get('/tools', authenticateToken, requireRole(['artisan']), 
   ArtisanController.getTools);
...
// View all job offers
router.get(
  '/view/jobs/offer/:limit',
  authenticateToken,
  requireRole(['artisan']),
  [
    param('limit').optional().isInt(),
  ],
  ArtisanController.getJobOffers,
);
